...
from PointTransformerV1 import PointTransformer

import logging
logger = logging.getLogger(__name__)

config_dict = {
    "architectures": ["PointLLMLlamaForCausalLM"],
    "bos_token_id": 1,
    "eos_token_id": 2,
    "hidden_act": "silu",
    "hidden_size": 4096,
...
class PointLLMLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if point_backbone is not None and (input_ids.shape[1] != 1 or self.training) and point_clouds is not None:
            ...
            #  [num group + 1, transdims] -> [B, 513, 384]
            ...
            # [num group + 1, project hidden dim] -> [B, 513, 2048]
            ...
            print("Input Embedded Embedding:", inputs_embeds.shape)
...
def test_PointLLMLlamaForCausalLM():
    config = PointLLMConfig(**config_dict)
    model = PointLLMLlamaForCausalLM(config)
    # Dummy data generation
    batch_size = 2
    seq_length = 500
    point_dim = 1024  # number of points
    num_point_features = 6  # x, y, z coordinates

    input_ids = torch.randint(0, config.vocab_size, (batch_size, seq_length))
    attention_mask = torch.ones_like(input_ids)
    point_clouds = torch.rand(batch_size, point_dim, num_point_features)

    labels = torch.randint(0, config.vocab_size, (batch_size, seq_length))

    # Run the forward pass
    outputs = model(input_ids=input_ids, attention_mask=attention_mask, point_clouds=point_clouds, labels=labels)

    # Check outputs
    assert outputs.loss is not None, "Loss should be computed."
    assert outputs.logits is not None, "Logits should be produced."
    assert outputs.logits.shape == (batch_size, seq_length, config.vocab_size), f"Logits should have shape {(batch_size, seq_length, config.vocab_size)}"

    print("Test passed with outputs:", outputs.logits.shape, "and loss:", outputs.loss.item())


test_PointLLMLlamaForCausalLM()