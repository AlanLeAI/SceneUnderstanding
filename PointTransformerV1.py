...
class LlamaPointModel(LlamaModel):
    def __init__(self, config, point_config):
        super(LlamaPointModel, self).__init__(config)
        self.point_transformer = PointTransformer(point_config)
        self.tokenizer = AutoTokenizer.from_pretrained("allenai/llama")

    def forward(self, input_ids, point_cloud_data):
        # Process point cloud data
        point_features = self.point_transformer(point_cloud_data)
        
        # Process text input
        text_features = self.get_input_embeddings()(input_ids)
        
        # Concatenate point features and text features
        combined_features = torch.cat([point_features, text_features], dim=1)
        
        # Pass combined features to the Llama model's decoder
        output = super().forward(inputs_embeds=combined_features)
        return output
...
def test_PointTransformer():
    xyz = torch.randn(32, 10000, 3).to("mps")
